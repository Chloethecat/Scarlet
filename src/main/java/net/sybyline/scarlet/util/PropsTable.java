...
public class PropsTable<E> extends JTable
{
    ...
    public Component prepareRenderer(javax.swing.table.TableCellRenderer renderer, int row, int column)
    {
        ...
        if (!this.isRowSelected(row))
        {
            ...
            if (modelRow >= 0 && modelRow < this.getPropsDataModel().entries.size())
            {
                E element = this.getPropsDataModel().entries.get(modelRow);
                Color overrideBg = this.getPropsTableExt().getOverrideBackgroundColor(element, c.getBackground(), row);
                if (overrideBg != null)
                    c.setBackground(overrideBg);
            }
        }
        return c;
    }
...
//        this.defaultRenderersByColumnClass.put(OffsetDate.class, (UIDefaults.LazyValue) t -> new PropsTemporalAccessorRenderer(FMT_OD));
    ...
    class PropsObjectRenderer extends PropsTableCellRenderer.PropsUIResource
    {
        ...
        protected void setValue(Object value)
        {
            ...
            Font overrideFont = PropsTable.this.getPropsTableExt().getOverrideFont(PropsTable.this.getPropsDataModel().entries.get(this.gtcrc_row), this.getFont(), this.gtcrc_column);
    ...
    class PropsBooleanRenderer extends JCheckBox implements TableCellRenderer, UIResource
    {
        ...
        public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column)
        {
            ...
            Font overrideFont = PropsTable.this.getPropsTableExt().getOverrideFont(PropsTable.this.getPropsDataModel().entries.get(row), this.getFont(), table.convertColumnIndexToModel(column));
            if (overrideFont != null)
            {
                this.setFont(overrideFont);
            }
    ...
    public class PropsTableExt
    {
        ...
        /** Column-aware variant. {@code column} is the model column index. */
        public Font getOverrideFont(E element, Font prev, int column)
        {
            return this.getOverrideFont(element, prev);
        }
        public Color getOverrideForegroundColor(E element, Color prev)
        {
            return null;
        }
        public Color getOverrideBackgroundColor(E element, Color prev, int row)
        {
            return null;
        }
    }
